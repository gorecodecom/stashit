...
export default function EditEntry({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/entries/${id}`)
      ...
      .then(data => {
        if (data.entry) {
          setEntry(data.entry)
          setImagePreview(data.entry.imageUrl)
        } else {
          router.push('/')
        }
      })
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const selectedCategories = Array.from(formData.getAll('categories')).map(String)
    
    const updatedEntry = {
      ...entry,
      title: formData.get('title'),
      content: formData.get('content'),
      imageUrl: imagePreview || entry?.imageUrl,
      url: formData.get('url')?.toString() || entry?.url,
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <select
                  ...
                  defaultValue={entry?.categories?.[0] || ''}
                >
